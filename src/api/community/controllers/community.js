...
module.exports = createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async create(ctx) {
      ...
      entity = await strapi.entityService.create("api::community.community", {
    ...
    async update(ctx) {
      ...
      if (ctx.is("multipart")) {
        ...
        if (!community) {
          ctx.throw(404, "community not found");
        }
        /* if (community?.admins.includes(user)) {
          ctx.throw(403, "You are not allowed to update this record");
        } */
        entity = await strapi.db.query("api::community.community").update({
          where: { id },
    ...
    async delete(ctx) {
      ...
      let entity;
      entity = await strapi.db.query("api::community.community").delete({
        where: { id },
      });
      const sanitizedEntity = await sanitize.contentAPI.output(entity);
      // console.log(sanitizedEntity);

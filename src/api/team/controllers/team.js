...
module.exports = createCoreController("api::team.team", ({ strapi }) => ({
  ...
  async create(ctx) {
    ...
    entity = await strapi.entityService.create("api::team.team", {
  ...
  async update(ctx) {
    ...
    if (ctx.is("multipart")) {
      ...
      entity = await strapi.db.query("api::team.team").update({
        where: { id },
  ...
  async delete(ctx) {
    ...
    let entity;
    entity = await strapi.db.query("api::team.team").delete({
      where: { id },
    });
    const sanitizedEntity = await sanitize.contentAPI.output(entity);
    // console.log(sanitizedEntity);
